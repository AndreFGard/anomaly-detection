...
A crescente adoção de robôs industriais em ambientes de manufatura e automação tem ampliado a demanda por sistemas confiáveis de monitoramento e detecção precoce de falhas. Em contextos industriais, anomalias operacionais podem resultar em danos físicos aos equipamentos, interrupções de produção, riscos à segurança humana e elevados custos de manutenção corretiva. Nesse cenário, a detecção automática de comportamentos anômalos a partir de dados sensoriais surge como uma ferramenta fundamental para viabilizar estratégias de manutenção preditiva e aumentar a robustez dos sistemas robóticos.
Este trabalho investiga o problema da detecção de anomalias em robôs industriais a partir de séries temporais de sensores inerciais (IMU), que capturam informações de aceleração, rotação e campo magnético nos eixos espaciais. Diferentes tipos de falhas mecânicas são considerados, incluindo colisões, sobrecarga mecânica e perturbações externas, as quais afetam de maneira distinta a dinâmica física do robô.
A motivação para este estudo decorre do uso crescente de robôs industriais em operações contínuas e de alta criticidade, nas quais esses sistemas executam tarefas físicas repetitivas, de alta precisão e frequentemente em proximidade com outros equipamentos e operadores. Nesse contexto, eventos anormais podem comprometer o funcionamento adequado do robô e provocar danos estruturais ao sistema e ao ambiente ao seu redor. A identificação automática de falhas contribui diretamente para a detecção de interferências externas e comportamentos anômalos, permitindo a mitigação de danos, a preservação da integridade do equipamento e a segurança do processo.
